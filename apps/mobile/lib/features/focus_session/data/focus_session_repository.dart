...
abstract interface class FocusSessionRepository {
  ...
  /// Mood and free text are encrypted before they touch the DB (PRD §24.3).
  /// [sessionId] is null for reflections not tied to a session (streak break).
...
class DriftFocusSessionRepository implements FocusSessionRepository {
  DriftFocusSessionRepository(this._db, this._clock, this._cipher);
  ...
  Future<void> saveReflection({
    ...
    final encryptedMood = moodTag == null
        ? null
        : await _cipher.encrypt(moodTag);
    final encryptedText = responseText == null
        ? null
        : await _cipher.encrypt(responseText);
