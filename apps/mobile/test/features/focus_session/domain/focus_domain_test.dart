...
void main() {
  ...
  group('FocusSession timing', () {
    ...
    test(
      'completed session reports planned duration and the ideal end time',
      () {
  ...
  group('Streak break detection', () {
    ...
    test('a dead run of 2+ days is a break', () {
      final streak = StreakCalculator.compute([day(5), day(4), day(3)], today);
      final broken = StreakCalculator.detectBreak(streak);
      expect(broken?.length, 3);
      expect(broken?.endedOn, day(3));
    });

    test('a live streak is not a break', () {
      final streak = StreakCalculator.compute([day(1), day(2)], today);
      expect(StreakCalculator.detectBreak(streak), isNull);
    });

    test('a 1-day run is too short to matter', () {
      final streak = StreakCalculator.compute([day(4)], today);
      expect(StreakCalculator.detectBreak(streak), isNull);
    });

    test('an acknowledged break is not reported again', () {
      final streak = StreakCalculator.compute([day(5), day(4), day(3)], today);
      expect(
        StreakCalculator.detectBreak(streak, acknowledgedRunEnd: day(3)),
        isNull,
      );
    });

    test(
      'a later break is reported even if an earlier one was acknowledged',
      () {
        ...
        final broken = StreakCalculator.detectBreak(
          streak,
          acknowledgedRunEnd: day(19),
        );
        expect(broken?.length, 3);
      },
    );
